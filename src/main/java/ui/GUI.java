...
public class GUI extends JFrame  {
    ...
    protected void updateView() {
        ...
        try {
            for (int y = 0; y < 2; y ++) {
                for (int x = 0; x < 6 ; x++) {
                    buttons[y][x].setText(Integer.toString(game.getNumStones(((-1 + y) * -12) + (x + y + ((-1 + y) * x * 2)))));
                }
            }
        } catch (PitNotFoundException err) {
            JOptionPane.showMessageDialog(this, err.getMessage(), "Pit Indexing Error Occured", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void newGame() {
        int selection = JOptionPane.showConfirmDialog(null, "<html>Starting a new game will delete current game.<br/>Would you like to save?<html/>", "Save Game?", JOptionPane.YES_NO_OPTION);
        if (selection == 0) {
            saveGame();
            restart();
        } else if (selection == 1) {
    ...
    private void saveGame() {
        ...
        try {
            if (saveFileName != null && saveFileName != ""){
                ...
                JOptionPane.showMessageDialog(this, "File Saved", "File Saved", JOptionPane.PLAIN_MESSAGE);
            }
        } catch (IOException err) {
            JOptionPane.showMessageDialog(this, "No file name provided", "Could Not Save File", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void savePlayer() {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(this, "No file name provided", "Could Not Save Player to File", JOptionPane.ERROR_MESSAGE);
        }
    }
    ...
    private void loadGame() {
        ...
        try {
            ...
            JOptionPane.showMessageDialog(this, "Something went wrong", "Could Not Load File", JOptionPane.ERROR_MESSAGE);
        } catch (ClassCastException err) {
            JOptionPane.showMessageDialog(this, "File does not contain a game", "Could Not Load File", JOptionPane.ERROR_MESSAGE);
        }
        updateView();
        pack();
    }
    ...
    private void loadPlayer() {
        ...
        try {
            ...
            if (choice == 1) {
                ...
                JOptionPane.showMessageDialog(this,"Cancel" ,"Canceling Player Load", JOptionPane.ERROR_MESSAGE);
            }
        } catch (IOException err) {
            JOptionPane.showMessageDialog(this, "Something went wrong", "Could Not Load File", JOptionPane.ERROR_MESSAGE);
        } catch (ClassCastException err) {
            JOptionPane.showMessageDialog(this, "File does not contain player profile", "Could Not Load File", JOptionPane.ERROR_MESSAGE);
        }
        updateView();
        pack();
    }
    ...
    private void makeMove(ActionEvent e) {
        ...
        try {
            game.move(((-1 + (clicked.getDown() - 1)) * -12) + ((clicked.getAcross() - 1) + (clicked.getDown() - 1) + ((-1 + (clicked.getDown() - 1)) * (clicked.getAcross() - 1) * 2)));
            updateView();
    ...
    protected void restart() {
        int newPlayers;
        makeChooseRules();
        newPlayers = JOptionPane.showOptionDialog(
        this,
        "Do you want to use new players?", 
        "New Players",
